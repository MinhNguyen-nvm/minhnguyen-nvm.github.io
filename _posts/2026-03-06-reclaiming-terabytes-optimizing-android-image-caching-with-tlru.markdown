...
tags: [app-disk, disk-size, optimization, scalability, android, caching, performance]
excerpt: "How we evolved standard LRU into Time-Aware LRU (TLRU) caching to reclaim terabytes of storage across millions of devices without degrading user experience."
...
The Grab Android app uses the [Glide library](https://github.com/bumptech/glide) as its primary image loading framework. Glide provides excellent features for efficient image loading, caching, and display. At its core, by default, Glide uses a cloned version of [Jake Wharton's DiskLruCache](https://github.com/JakeWharton/DiskLruCache) for disk-based caching.
...
Figure 1 and 2 illustrate how LRU cache trimming works. These diagrams present an LRU cache with a maximum size of 100 MB containing three cache entries totaling 95 MB. When a new 25 MB cache entry is added, it exceeds the cache's maximum size.

![A new cache entry is added to an LRU cache that's near its 100 MB capacity, exceeding the limit.](/assets/images/tlru/figure-1.png)
*Figure 1. A new cache entry is added to an LRU cache that's near its 100 MB capacity, exceeding the limit.*

![The LRU cache automatically trims the least recently used entry to bring the total size back within the 100 MB limit.](/assets/images/tlru/figure-2.png)
...
![Initial TLRU cache state and the impact of adding new entries.](/assets/images/tlru/figure-3.png)
...
![Size-based eviction removes the least recently used entry to enforce storage limits.](/assets/images/tlru/figure-4.png)
...
![Time-based eviction and minimum threshold protection working together.](/assets/images/tlru/figure-5.png)
...
![Example of the journaling mechanism in DiskLruCache.](/assets/images/tlru/figure-6.png)
*Figure 6. Example of the journaling mechanism in DiskLruCache.*

**Journal file format** — the journal file is a plain text file that records cache operations line by line:
...
Key aspects of the journaling mechanism:
...
1. Tracking last access time
2. Time-based eviction logic
...
![Example of a TLRU journal file.](/assets/images/tlru/figure-7.png)
...
![TLRU eviction decision flow - evaluating cache entries based on time expiration and size constraints.](/assets/images/tlru/figure-8.png)
